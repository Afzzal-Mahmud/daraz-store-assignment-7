...
const loadProducts = () => {
  ...
  function rating(rating) {
    console.log(rating);
    const userRating = rating.rate;
    const rateCount = rating.count;
    return `Rating: ${userRating}  RateCount: ${rateCount}`
  }
  const productContainetr = document.getElementById("all-products")
  // show all product in UI 
  const showProducts = (products) => {
  
    const allProduct = products;
    allProduct.forEach(singelProduct => {
  
      const div = document.createElement("div");
      div.innerHTML = 
      `<div class="single-product">
      <div>
      <img class="product-image" src=${singelProduct.image}></img>
      </div>
      <h3>${singelProduct.title}</h3>
      <p>Category: ${singelProduct.category}</p>
      <h4 class="py-2">${rating(singelProduct.rating)}</h4>
      <h3>Price: $ ${singelProduct.price}</h3>
      <button onclick="addToCart(${singelProduct.id},
        ...
        console.log(`${singelProduct.rating}`)
    });
  };
  let count = 0;
  ...
  //getInput value take a HTML id and return inner Text as number
